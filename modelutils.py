...
from transformers import AutoConfig, AutoModelForCausalLM, MoonshineForConditionalGeneration

from quant_groups import dequantize

MODEL_ERROR_MSG = "Unsupported model type {} - only 'llama', 'Yi', 'opt', 'falcon' and 'moonshine' are supported"
...
def get_model(model_path, load_quantized=None, dtype="auto"):
    ...
    # Detect if model is moonshine (encoder-decoder) vs causal LM
    config = AutoConfig.from_pretrained(model_path, trust_remote_code=True)
    is_moonshine = getattr(config, "model_type", "") == "moonshine"

    with suspend_nn_inits():
        if load_quantized:
            print("Initializing model with random weights...")
            if is_moonshine:
                model = MoonshineForConditionalGeneration(config).to(dtype=dtype).eval()
            else:
...
def get_model_head(model):
    ...
    if model.config.model_type in LLAMA_LIKE:
        ...
        head.append(model.lm_head)
    elif model.config.model_type == "moonshine":
        if model.model.decoder.norm is not None:
            head.append(model.model.decoder.norm)
        head.append(model.proj_out)
    else:
...
def get_lm_logits(inps_, model):
    if model.config.model_type in LLAMA_LIKE:
        ...
        lm_logits = model.proj_out(hidden_states)
    else:
        raise ValueError(MODEL_ERROR_MSG.format(model.config.model_type))
    return lm_logits
...
def get_decoder_layers(model):
    """Return decoder layers for encoder-decoder models (moonshine)."""
    if model.config.model_type == "moonshine":
        return model.model.decoder.layers
    else:
        raise ValueError("get_decoder_layers is only supported for moonshine models")
...
def get_decoder_sequential_groups(model):
    """Return sequential groups for decoder layers of encoder-decoder models (moonshine)."""
    if model.config.model_type == "moonshine":
        return [
            ["self_attn.q_proj", "self_attn.k_proj", "self_attn.v_proj"],
            ["self_attn.o_proj"],
        ...
        raise ValueError("get_decoder_sequential_groups is only supported for moonshine models")
