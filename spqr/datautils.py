...
def get_audio_calibration_data(nsamples, audio_len=160000, seed=0):
    """
    Generate synthetic audio calibration data for Moonshine models.
    Returns a list of tensors of shape (1, audio_len) representing raw 16kHz audio waveforms.

    Args:
        nsamples (int): Number of audio samples to generate.
        audio_len (int): Length of each audio waveform in samples (default 160000 = 10s at 16kHz).
...
def get_loaders(name, nsamples=128, seed=0, seqlen=2048, eval_mode=False, model_path=None):
    ...
    # audio calibration data for moonshine models
    if name.lower() == "audio_random":
        data = get_audio_calibration_data(nsamples, audio_len=seqlen, seed=seed)
