...
from typing import Union, Dict, Iterable, Optional, Tuple, List
...
class LinearLowRank(nn.Module):
    def __init__(self, in_feature, out_feature, rank):
        super().__init__()
        self.weight1 = mx.array((in_feature, rank))
        self.weight2 = mx.array((rank, out_feature))
        self.bias = mx.array(out_feature)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_state: int, n_head: int, low_rank_config: Dict[str, int] = None):
        super().__init__()
        self.n_head = n_head
        if low_rank_config is not None:
            self.query = LinearLowRank(n_state, n_state, low_rank_config["q_proj"]) \
                if "q_proj" in low_rank_config else nn.Linear(n_state, n_state)
            self.key = LinearLowRank(n_state, n_state, low_rank_config["k_proj"]) \
                if "k_proj" in low_rank_config else nn.Linear(n_state, n_state, bias=False)
            self.value = LinearLowRank(n_state, n_state, low_rank_config["v_proj"]) \
                if "v_proj" in low_rank_config else nn.Linear(n_state, n_state)
            self.out = LinearLowRank(n_state, n_state, low_rank_config["out_proj"]) \
                if "out_proj" in low_rank_config else nn.Linear(n_state, n_state)
        else:
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, n_state: int, n_head: int, cross_attention: bool = False, low_rank_config: Dict[str, int] = None):
        super().__init__()

        self.attn = MultiHeadAttention(n_state, n_head, low_rank_config)
        ...
        n_mlp = n_state * 4
        if low_rank_config is not None:
            self.mlp1 = LinearLowRank(n_state, n_mlp, low_rank_config["fc1"]) \
                if "fc1" in low_rank_config else nn.Linear(n_state, n_mlp)
            self.mlp2 = LinearLowRank(n_mlp, n_state, low_rank_config["fc2"]) \
                if "fc2" in low_rank_config else nn.Linear(n_mlp, n_state)
...
class AudioEncoder(nn.Module):
    def __init__(
        ...
        self.blocks = [ResidualAttentionBlock(n_state, n_head, low_rank_config=low_rank_config[i]) for i in range(n_layer)]
...
class Whisper(nn.Module):
    def __init__(self, dims: ModelDimensions, dtype: mx.Dtype = mx.float16, low_rank_config: List[Dict[str, int]] = None):
        super().__init__()
        self.dims = dims
